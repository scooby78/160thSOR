/*
	Name: 160th SOR Mod
	Author: Scooby
	Date: 13/07/16 01:09
	Description: cfgPatches.hpp
*/

class CfgPatches 
{
	class 160th_SOR_Loadouts 
	{
		author="160th SOR";
		url="http://www.160th.net";
		units[] = 
		{
			"SOR_Base",
			"SOR_Actual_D",
			"SOR_Commander_D",		
			"SOR_MechCommand_D",
			"SOR_AirCommand_D",
			"SOR_RTO_D",	
			"SOR_TeamLeader_D",
			"SOR_TeamLeader2_D",
			"SOR_Medic_D",
			"SOR_HMedic_D",
			"SOR_M249AR_D",
			"SOR_M240AR_D",
			"SOR_Grenadier_D",
			"SOR_Rifleman_D",
			"SOR_Rifleman_ammo_D",			
			"SOR_RiflemanAT_D",
			"SOR_HeliPilot_D",
			"SOR_HeliCrew_D",
			"SOR_JetPilot_D",
			"SOR_MEVPilot_D",
			"SOR_ParaJumper_D",
			"SOR_ReconLeader_D",
			"SOR_ReconJTAC_D",
			"SOR_ReconRifleman_D",	
			"SOR_Recon_M249AR_D",
			"SOR_ReconSpotter_D",
			"SOR_Marksman_D",	
			"SOR_Spotter_D",
			"SOR_Sniper_D",	
			"SOR_MechCrew_D",
			"SOR_MechOperator_D",
			"SOR_MechCrewCommander_D",
			"SOR_MechDriver_D",
			"SOR_HMGActual_D",			
			"SOR_HMGGunner_D",			
			"SOR_HMGCarrier_D",
			"SOR_HMGRFL_D",
			"SOR_MORActual_D",
			"SOR_MORGunner_D",			
			"SOR_MORCarrier_D",			
			"SOR_MORRFL_D",
			"SOR_Engineer_Teamleader_D",
			"SOR_Engineer_D",
			
			"SOR_Actual_W",
			"SOR_Commander_W",
			"SOR_MechCommand_W",
			"SOR_AirCommand_W",
			"SOR_RTO_W",			
			"SOR_Teamleader_W",
			"SOR_Teamleader2_W",
			"SOR_Medic_W",
			"SOR_HMedic_W",
			"SOR_M249AR_W",
			"SOR_M240AR_W",
			"SOR_Grenadier_W",
			"SOR_Rifleman_W",
			"SOR_Rifleman_ammo_W",
			"SOR_RiflemanAT_W",
			"SOR_HeliPilot_W",
			"SOR_HeliCrew_W",
			"SOR_JetPilot_W",
			"SOR_MEVPilot_W",
			"SOR_ParaJumper_W",	
			"SOR_ReconLeader_W",
			"SOR_ReconJTAC_W",
			"SOR_ReconRifleman_W",
			"SOR_Recon_M249AR_W",
			"SOR_ReconSpotter_W",
			"SOR_Marksman_W",			
			"SOR_Sniper_W",
			"SOR_Spotter_W",
			"SOR_MechCrew_W",
			"SOR_MechOperator_W",
			"SOR_MechCrewCommander_W",
			"SOR_MechDriver_W",
			"SOR_HMGActual_W",
			"SOR_HMGGunner_W",
			"SOR_HMGCarrier_W",
			"SOR_HMGRFL_W",
			"SOR_MORActual_W",
			"SOR_MORGunner_W",
			"SOR_MORCarrier_W",
			"SOR_MORRFL_W",
			"SOR_Engineer_W",
			
			"SOR_RHS_M6_wd",
			"SOR_RHS_M2A3_BUSKIII_wd",
			"SOR_rhsusf_m113_usarmy",
			"SOR_rhsusf_m113_usarmy_supply",
			"SOR_rhsusf_m113_usarmy_unarmed",
			"SOR_rhsusf_m113_usarmy_medical",
			"SOR_rhsusf_m1a1aimd_usarmy_wd",
			"SOR_rhsusf_m1a1aim_tuski_wd",
			"SOR_rhsusf_m1a2sep1d_usarmy_wd",
			"SOR_rhsusf_m1a2sep1tuskiwd_usarmy",
			"SOR_rhsusf_m1a2sep1tuskiiwd_usarmy",
			"SOR_B_SDV_01_F_wd",
			"SOR_B_Boat_Transport_01_F_wd",
			"SOR_rhsusf_M1083A1P2_wd_fmtv_usarmy",
			"SOR_rhsusf_m1025_w",
			"SOR_rhsusf_m1025_w_Mk19",
			"SOR_rhsusf_m1025_w_m2",
			"SOR_rhsusf_m998_w_4dr",
			"SOR_rhsusf_m998_w_2dr",
			"SOR_rhsusf_m1025_w_eng",
			"SOR_rhsusf_M1083A1P2_wd_fmtv_usarmy_eng",
			"SOR_Eng_Truck_ammo_wd",
			"SOR_Eng_Truck_fuel_wd",
			"SOR_Eng_Truck_Repair_wd",
			"SOR_Eng_APC_Tracked_01_wd",
			"SOR_B_APC_Tracked_02_wd",
			"SOR_UH60M_W",
			"SOR_CH_47F_W",
			"SOR_UH60M_MEV2_W",
			"SOR_MELB_H6M_W",
			"SOR_MELB_MH6M_W",
			"SOR_MELB_MH6M_MEV_W",
			"SOR_MELB_AH6M_L_W",
			"SOR_MELB_AH6M_M_W",
			"SOR_MELB_AH6M_H_W",
			"SOR_RHS_AH64D_AA_W",
			"SOR_RHS_AH64D_CS_W",
			"SOR_RHS_AH64D_GS_W",
			"SOR_RHS_A10_W",
			"SOR_FIGHTER_W",
			"SOR_STRIKE_FIGHTER_W",
			
			"SOR_Opfor_Rifleman_D",
			"SOR_OpFor_PKM_D",
			"SOR_Opfor_TL_D",
			"SOR_Land_HelipadCircle_F",
			"SOR_HelipadRescue_F",
			"SOR_MechService",
			"SOR_Land_Medevac_HQ_V1_F",
			"SOR_Land_Radar",
			"SOR_Land_Screen",
			"SOR_Land_Flush_Light_green_F",
			"SOR_Land_Flush_Light_red_F",
			"SOR_Land_Flush_Light_yellow_F",
			"SOR_Land_PortableLight_double_F",
			"SOR_PJ_Box_F",
			"SOR_PJ_Pack_Box_F",
			"SOR_Air_LP",
			"SOR_A_Box_F",
			"SOR_M_Box_F",
			"SOR_E_Box_F",
			"SOR_G_Box_F",
			"SOR_S_Box_F",
			"SOR_divegear",
			"SOR_HW_Box_F",	
			"SOR_Box_NATO_WpsSpecial_F",
			"SOR_Box_NATO_WpsSpecial_F_Training",			
			"SOR_A_Crate_F",
			"SOR_AM_Crate_F",
			"SOR_HMG_Crate_F",
			"SOR_MOR_Crate_F",
			"SOR_Land_Pod_Heli_Transport_04_ammo_F",
			"SOR_Land_Pod_Heli_Transport_04_medevac_F",			
			"SOR_Land_Pod_Heli_Transport_04_bench_F",
			"SOR_Land_Pod_Heli_Transport_04_covered_F",
			"SOR_VTOL_Troop_D",
			"SOR_UH60M",			
			"SOR_CH_47F",
			"SOR_UH60M_MEV2",
			"SOR_MELB_H6M",
			"SOR_MELB_MH6M",
			"SOR_MELB_MH6M_MEV",			
			"SOR_MELB_AH6M_L",
			"SOR_MELB_AH6M_M",	
			"SOR_MELB_AH6M_H",
			"SOR_RHS_AH64D_AA",		
			"SOR_RHS_AH64D_CS",
			"SOR_RHS_AH64D_GS",	
			"SOR_RHS_A10",
			"SOR_FIGHTER",
			"SOR_STRIKE_FIGHTER",
			"SOR_O_Heli_Transport_04_F",
			"SOR_O_Heli_Transport_04_medevac_F",
			"SOR_O_Heli_Transport_04_bench_F",
			"SOR_O_Heli_Transport_04_covered_F",
			"SOR_RHS_Mi8amt_civilian",
			"SOR_O_Heli_Attack_02_F",
			"SOR_O_Heli_Attack_02_black_F",
			"SOR_O_Heli_Light_02_v2_F",
			"SOR_O_Plane_CAS_02_F",
			"SOR_I_Plane_Fighter_03_AA_B",
			"SOR_I_Plane_Fighter_03_CAS_B",
			"SOR_FIR_F16C_OPFOR",
			"SOR_FIR_F16C_TWAS2",
			"SOR_I_Plane_Fighter_03_AA_F",
			"SOR_I_Plane_Fighter_03_CAS_F",
			"SOR_RHS_M6",
			"SOR_M2A3_BUSKIII",
			"SOR_rhsusf_m113d_usarmy",			
			"SOR_rhsusf_m113d_usarmy_supply",
			"SOR_rhsusf_m113d_usarmy_unarmed",
			"SOR_rhsusf_m113d_usarmy_medical",	
			"SOR_rhsusf_m1a1aimd_usarmy",
			"SOR_rhsusf_m1a1aim_tuski_d";
			"SOR_rhsusf_m1a2sep1d_usarmy",
			"SOR_rhsusf_m1a2sep1tuskid_usarmy",
			"SOR_rhsusf_m1a2sep1tuskiid_usarmy",
			"SOR_B_SDV_01_F",
			"SOR_B_Boat_Transport_01_F",
			"SOR_rhsusf_M1083A1P2_d_fmtv_usarmy",
			"SOR_rhsusf_m1025_d",
			"SOR_rhsusf_m1025_d_Mk19",			
			"SOR_rhsusf_m1025_d_m2",
			"SOR_rhsusf_m998_d_4dr",
			"SOR_rhsusf_m998_d_2dr",
			"SOR_Eng_Car_Transport_D",			
			"SOR_Eng_Truck_Transport_D",			
			"SOR_Eng_Truck_ammo_D",
			"SOR_Eng_Truck_fuel_D",
			"SOR_Eng_Truck_Repair_D",
			"SOR_Eng_APC_Tracked_01",
			"SOR_B_APC_Tracked_02",	
			"SOR_B_APC_Wheeled_01_cannon_F"
		};
		weapons[] = 
		{
			"160_weap_pj",
			"160_weap_inf_std",
			"160_weap_inf_tl",
			"160_weap_recon",
			"160_weap_inf_m249",
			"160_weap_inf_m240",
			"160_weap_inf_gl",
			"160_rhs_weap_ak74m_pkas",
			"160_rhs_weap_m4a1_carryhandle_grip2",
			"160_m4a1_carryhandle_grip_holo",			
			"160_m4a1_carryhandle_grip_acog",
			"160_m4a1_grip2",
			"160_m4a1_m320",
			"160_rhs_weap_m249_pip_L_para",
			"160_rhs_weap_m240B",
			"160_srifle_LRR_camo_F",
			"160_rhs_weap_sr25",
			"160_m4a1_blockII_M203",
			"160_m4a1_blockII_grip2_KAC",
			"160_rhs_weap_m16a4_carryhandle",
			"160_rhs_weap_sr25",			
			"160_rhs_weap_m16a4_carryhandle_grip_pmag",
			"160_rhs_weap_m16a4_carryhandle_grip_pmag_1",
			"160_rhs_weap_m4a1_carryhandle_grip2_1"			
		};
		version="2.2.2";
//		requiredVersion = 0.1;
		requiredAddons[] = 
		{
			"rhsusf_main",
			"rhsusf_c_weapons",
			"A3_Characters_F",
			"ace_medical",
			"ace_common",
			"task_force_radio_items"
		};
	};
};
