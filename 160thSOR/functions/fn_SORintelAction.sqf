...
nul = ['Explosion_type','Number_of_intel_pieces'] spawn SOR_fnc_SORintelAction;
E.g. nul = ['1',2] spawn SOR_fnc_SORintelAction;
*/
...
switch (_mode) do {
	...
	if (isnil "Intel1") exitWith {hint "No intel object detected... \n  1).Check you have placed an object on map \n 2). Check the object has the naming syntax 'intel1'"};
		Intel1 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel1,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,1] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP1""))"];
	};
    case 2:
	{ 
		...
		Intel1 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel1,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,1] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP1""))"];
		Intel2 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel2,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,2] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP2""))"];
	};
    case 3:
	{ 
		...
		Intel1 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel1,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,1] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP1""))"];
		Intel2 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel2,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,2] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP2""))"];
		Intel3 addAction [("<t color='#16e616'>" + ("Retrieve Intel") + "</t>"),"[Intel3,SOR_INTELBOMBTIMER,SOR_INTELBOMBTYPE,3] execVM 'selfdestruct.sqf'","",1,true,true,"","((_target distance _this) < 5 && (isNil ""SOR_INTELPICKEDUP3""))"];
	};
};
